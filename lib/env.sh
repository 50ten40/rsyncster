...
ignore_files=(all default permatecture.pro faf.photos braingurus.com signup.fafchat.com signup.faf.chat signup.faf.photos rodpweiss.com signup.faf.social signup.mastery.chat) # used for drupalfiles_get.sh

export ignore_files DRUPAL_WEBHEAD_PATH DRUPAL_MULTISITE_DOMAIN DRUPAL_DEV_DOMAINS DEBUG SCHEME PREFIX NPREFIX HAPREFIX LOADBALANCER CHANGESSTRING LIVEDIR DOCROOTDIR STAGINGDIR LIVEDIR WORKINGDIR DOMAINSFILE RSYNCSTERSCRIPT DRUPALCACHE APPSERVERS APPSERVERSSHORTNAME APPSERVERSMASTER LIBDIR LOGDIR WEBUSER status exclusions waittime stagingservers webservers
